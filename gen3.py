...
# --- 設定ここから --- #
# 生成する記事のテーマ数
NUM_THEMES_TO_GENERATE = 15

# テーマ考案の背景情報 (適宜編集してください)
THEME_GENERATION_CONTEXT = """
英語学習に本気で取り組む層の大学生/大学院生に向けて、実践的なアドバイスや豆知識、役立つ情報を提供したい。
例：「TOEFLは誰が採点しているのか」、「アイビーリーグとは」、「EA(Early Action)とは」、「TOEICは主にどんな人が受けるのか」など、特定のニッチな疑問または悩みに対して、具体的な回答を提供したい。
"""
# --- 設定ここまで --- #
...
- **スタイルと強調**: 文章の重要な部分には、`<strong>` タグによる太字強調だけでなく、必要に応じてReactのインラインスタイルを用いた色文字などの装飾を推奨します。これにより、視覚的なアクセントを加え、読者の理解を助け、記事の魅力を高めることができます。
    ...
    - 使用する色やスタイルは、記事全体で一貫性を持たせ、ウェブサイト全体のデザイントーンから大きく逸脱しないように注意してください。**基本的な強調では、#2563ebを使用してください。**
