...
海外大学または大学院への進学を目指している学生をターゲットにした、各大学の紹介記事を作成したい。
必ずしも一つの記事が一つの大学に対応しなくてもいいが、一つの記事に複数の大学を紹介する場合は、関連性が明確になるようにしてください。(基本は一つの大学の紹介記事を作成してください。)
***特定の大学の紹介以外のテーマは絶対に選ばないでください。***
**参考にしてほしい例：**
「スタンフォード大学はどこがすごい？」、「ケンブリッジ大学ってどのくらい難しい？」、「UCLAとは？」
"""
# --- 設定ここまで --- #
...
- 記事を説明する4単語以内(できるだけ2単語以内)のタイトルを挿入するように指示すること。(例："TOEIC Listening"、"Harvard University"など)
その文字列をプロンプトで明示し、**目に入りやすいように非常に大きく、見やすく、デザインも洗練されたフォントで表示されるように**指示すること。
それ以外の文字列は極力含まれないように指示すること。
