...
function SignInForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        setErrorMessage('');
        try {
            await (0, auth_firebase_1.loginUser)(email, password);
            router.push('/mypage');
        }
        catch (error) {
    ...
    return (<div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-900">
          または{' '}
          <link_1.default href="/auth/signup" className="font-medium text-blue-600 hover:text-blue-500">
            新規登録
          </link_1.default>
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          {registered && (<div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded mb-4">
              アカウントが作成されました。ログインしてください。
            </div>)}

          {errorMessage && (<div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded mb-4">
              {errorMessage}
            </div>)}
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <link_1.default href="/auth/forgot-password" className="font-medium text-blue-600 hover:text-blue-500">
                  パスワードをお忘れですか？
                </link_1.default>
              </div>
            </div>
