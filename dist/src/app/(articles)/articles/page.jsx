...
exports.default = ArticlesHomePage;
...
const article_data_1 = require("../lib/article-data");
...
function ArticlesHomePage() {
    ...
    const allArticles = (0, article_data_1.getSortedArticlesData)(); // Use the imported function
    const searchParams = (0, navigation_1.useSearchParams)();
    const router = (0, navigation_1.useRouter)();
    const [selectedCategory, setSelectedCategory] = (0, react_2.useState)(null);
    ...
    return (<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {selectedCategory === null && searchTerm === '' && (<>
              ...
              <div className="grid md:grid-cols-2 gap-0">
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <link_1.default href="/articles/toeic-mocktest-comparison" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-md transition-colors">
                      人気記事を読む
                    </link_1.default>
                    <link_1.default href="/" className="px-6 py-3 bg-white hover:bg-slate-100 text-slate-800 font-medium rounded-lg shadow-md transition-colors">
                      ToreMockで模試を受ける
                    </link_1.default>
                  </div>
                </div>
                <div className="hidden md:block relative h-full min-h-[300px] bg-slate-700">
                  <image_1.default src="/images/toeic-comparison.jpg" alt="英語学習をしている女性" fill className="object-cover" priority/>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {categories.map(category => (<div key={category} className="bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow border border-slate-200">
                  ...
                  <button onClick={() => handleCategoryChange(category)} className="w-full py-2 px-4 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition-colors font-medium text-sm text-center">
                    {category}の記事を見る
                  </button>
                </div>))}
            </div>
          ...
          {/* 人気記事 */}
          {popularArticles.length > 0 && (<div className="mb-16">
              <h2 className="text-2xl font-bold text-slate-800 mb-6">
                <span className="inline-block w-3 h-8 bg-blue-600 rounded-full mr-3"></span>
                人気の記事
              </h2>
              ...
              <div className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
                {popularArticles.map((article, index) => (<div key={article.id} className={`flex flex-col md:flex-row ${index < popularArticles.length - 1 ? 'border-b border-slate-200' : ''}`}>
                    <div className="relative md:w-1/4 h-40 md:h-auto bg-slate-100 flex-shrink-0"> 
                      <image_1.default src={article.imageSrc || '/images/placeholder.jpg'} alt={article.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 25vw"/>
                    </div>
                    <div className="p-6 md:w-3/4">
                      ...
                      <div className="flex flex-wrap justify-between items-center">
                        <div className="flex flex-wrap gap-2 mb-2 md:mb-0">
                          {article.tags.map(tag => (<span key={tag} className="px-2 py-1 bg-slate-100 text-slate-600 text-xs rounded-full">
                              #{tag}
                            </span>))}
                        </div>
                        <link_1.default href={`/articles/${article.id}`} className="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 transition-colors text-sm font-medium">
                          記事を読む
                        </link_1.default>
                      </div>
                    </div>
      ...
      <div className="mb-16">
        ...
        <div className="flex flex-wrap justify-center mb-10">
          <button onClick={() => handleCategoryChange(null)} className={`m-1 px-4 py-2 text-sm font-medium rounded-full transition-colors ${selectedCategory === null
            ? 'bg-slate-800 text-white'
            : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>
            すべて
          </button>
          {categories.map(category => (<button key={category} onClick={() => handleCategoryChange(category)} className={`m-1 px-4 py-2 text-sm font-medium rounded-full transition-colors ${selectedCategory === category
                ? 'bg-slate-800 text-white'
                : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>
              {category}
            </button>))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredArticles.length > 0 ? (filteredArticles.map(article => (<div key={article.id} className={`border border-slate-200 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 bg-white`}>
                {/* 画像部分 - Adjust aspect ratio here: remove h-48, add aspect-[3/2] */}
                <div className="relative bg-slate-200 aspect-[3/2]">
                  <image_1.default src={article.imageSrc || '/images/placeholder.jpg'} alt={article.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
                  {article.featured && (<div className="absolute top-2 right-2 px-2 py-1 bg-blue-600 text-white text-xs font-medium rounded">
                      注目記事
                    </div>)}
                </div>
                ...
                <div className="p-5 flex flex-col h-[calc(100%-12rem)]">
                  ...
                  <link_1.default href={`/articles/${article.id}`} className="block text-center py-2 bg-slate-800 text-white rounded-md font-medium hover:bg-slate-700 transition-colors mt-auto">
                    記事を読む
                  </link_1.default>
                </div>
              </div>))) : (<div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
              <p className="text-xl text-slate-600 mb-4">
                {searchTerm
                ? `「${searchTerm}」に一致する記事は見つかりませんでした。`
                : '該当する記事は見つかりませんでした。'}
              </p>
              <button onClick={() => router.push('/articles')} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-md hover:bg-slate-300 transition-colors">
                記事一覧に戻る
              </button>
            </div>)}
        </div>
      ...
      <div className="mb-16 bg-gradient-to-r from-slate-700 to-slate-900 rounded-xl p-8 shadow-lg text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <link_1.default href="/" className="inline-block px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-md transition-colors">
            模試を受けてみる
          </link_1.default>
        </div>
      </div>
      ...
      <div className="mb-16 bg-white rounded-xl p-8 shadow-md border border-slate-200">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-2 max-w-md mx-auto">
            <input type="email" placeholder="メールアドレスを入力" className="flex-grow px-4 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500"/>
