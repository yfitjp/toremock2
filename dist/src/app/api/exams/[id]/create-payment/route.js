...
async function POST(req, { params }) {
    try {
        ...
        // 模試の存在確認
        const examRef = firebase_admin_2.db.collection('exams').doc(params.id);
        ...
        if (!stripeProductId || !stripePriceId) {
            ...
            return new server_1.NextResponse('Stripeの商品設定が不正です', { status: 400 });
        }
        // 既に購入済みかチェック
        const existingPurchase = await firebase_admin_2.db.collection('purchases')
            .where('userId', '==', userId)
        ...
        if (!existingPurchase.empty) {
            ...
            if (purchaseData.status === 'completed') {
                return new server_1.NextResponse('この模試は既に購入済みです', { status: 400 });
            }
            // pendingの場合は古い購入レコードを削除（または非アクティブにマーク）
            if (purchaseData.status === 'pending') {
                console.log('古いpending状態の購入レコードを削除します:', purchaseDoc.id);
                await firebase_admin_2.db.collection('purchases').doc(purchaseDoc.id).update({
                    status: 'abandoned',
                    updatedAt: new Date()
                });
            }
        ...
        if (!process.env.STRIPE_SECRET_KEY) {
            ...
            return new server_1.NextResponse('Stripeの設定が不正です', { status: 500 });
        }
        if (!process.env.NEXT_PUBLIC_API_URL) {
            console.error('NEXT_PUBLIC_API_URL is not set');
            return new server_1.NextResponse('API URLの設定が不正です', { status: 500 });
        }
        // 既存の顧客を検索または新規作成
        let customer;
        try {
            const customers = await stripe_server_1.stripe.customers.search({
            ...
            if (customers.data.length > 0) {
                ...
                customer = await stripe_server_1.stripe.customers.create({
            ...
            return new server_1.NextResponse('顧客の作成に失敗しました', { status: 500 });
        }
        // Stripeの価格情報を取得
        const price = await stripe_server_1.stripe.prices.retrieve(stripePriceId);
        if (!price) {
            return new server_1.NextResponse('価格情報の取得に失敗しました', { status: 500 });
        }
        const amount = price.unit_amount || 0;
        // PaymentIntentを作成
        const paymentIntent = await stripe_server_1.stripe.paymentIntents.create({
            amount,
            currency: 'jpy',
        ...
        await firebase_admin_2.db.collection('purchases').add({
            userId,
            examId: params.id,
