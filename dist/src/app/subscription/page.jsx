...
const subscriptions_2 = require("@/app/lib/subscriptions");
...
function CheckoutForm({ onSuccess }) {
    ...
    const [paymentMessage, setPaymentMessage] = (0, react_1.useState)('');
    (0, react_1.useEffect)(() => {
        if (!stripe) {
            return;
        }
    ...
    return (<form onSubmit={handleSubmit} className="space-y-6">
      ...
      {paymentMessage && (<div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-md">
          {paymentMessage}
        </div>)}

      <button type="submit" disabled={!stripe || processing} className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${(!stripe || processing) ? 'opacity-50 cursor-not-allowed' : ''}`}>
...
function SubscriptionPage() {
    ...
    const [checkingSubscription, setCheckingSubscription] = (0, react_1.useState)(true);
    ...
    const [paymentProcessing, setPaymentProcessing] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
        const checkSubscription = async () => {
            if (user) {
                try {
                    console.log('SubscriptionPage: サブスクリプションチェック開始 - ユーザーID:', user.uid);
                    const hasSubscription = await (0, subscriptions_2.hasActiveSubscription)(user.uid);
    ...
    const handleSubscribe = async () => {
        ...
        try {
            ...
            if (data.sessionUrl) {
                ...
                setPaymentAmount(data.amount || subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.price);
            }
            else {
    ...
    const handlePaymentSuccess = async () => {
        ...
        try {
            ...
            if (userDoc.exists()) {
                ...
                await (0, firestore_1.updateDoc)(userRef, updateData);
    ...
    const handlePaymentIntent = async (result) => {
        if (result.error) {
            // エラー処理
    ...
    if (!user) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="text-center">
            ...
            <div className="mt-8">
              <link_1.default href="/auth/signin" className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                ログイン
              </link_1.default>
            </div>
          </div>
    ...
    return (<div className="bg-white min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        {hasSubscription ? (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mt-8 bg-lime-50 border border-lime-200 rounded-lg p-6 max-w-3xl mx-auto">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-green-800">現在のプラン: {subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.name}</h3>
            ...
            <div className="mt-4 border-t border-lime-200 pt-4">
              ...
              <ul className="mt-2 space-y-2">
                {subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.features.map((feature, index) => (<li key={index} className="flex items-start">
                    <svg className="flex-shrink-0 w-5 h-5 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div className="mt-6">
              <link_1.default href="/exams" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                模試一覧を見る
              </link_1.default>
            </div>
        ...
        <div className={`mt-12 ${hasSubscription ? 'opacity-60' : ''} space-y-12 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-x-8`}>
          {/* 無料プラン */}
          <framer_motion_1.motion.div className="relative p-8 bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col">
            ...
            <div className="flex items-center">
              ...
              <h3 className="text-3xl font-semibold text-gray-900">{subscriptions_1.SUBSCRIPTION_PLANS.FREE.name}</h3>
            </div>
              ...
              <ul role="list" className="mt-6 space-y-4">
                {subscriptions_1.SUBSCRIPTION_PLANS.FREE.features.map((feature, index) => (<li key={index} className="flex">
                    <svg className="flex-shrink-0 w-6 h-6 text-gray-900" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <link_1.default href="/exams" className="mt-8 block w-full bg-gray-100 border border-transparent rounded-md py-3 px-6 text-center font-bold text-gray-900 hover:bg-gray-200">
              模試一覧を見る
            </link_1.default>
          </framer_motion_1.motion.div>

          {/* プレミアムプラン */}
          <framer_motion_1.motion.div className="relative p-8 bg-gradient-to-b from-lime-50 to-lime-200 border border-lime-200 rounded-2xl shadow-md flex flex-col transform">
            <div className="flex-1">
              <div className="flex items-center">
                <div className="w-8 h-8 mr-2">
              ...
              <p className="mt-4 flex items-baseline text-green-900">
                <span className="text-5xl font-extrabold tracking-tight">¥{subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.price.toLocaleString()}</span>
                <span className="ml-1 text-xl font-semibold">/月</span>
              </p>
              <div className="mt-1">
              ...
              <p className="mt-6 text-green-900">
                {subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.description} エリートプランと同様の高品質な学習体験を、よりリーズナブルな価格でご提供します。
              </p>

              <ul role="list" className="mt-6 space-y-4">
                {subscriptions_1.SUBSCRIPTION_PLANS.PREMIUM.features.map((feature, index) => (<li key={index} className="flex">
                    <svg className="flex-shrink-0 w-6 h-6 text-green-900" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/>
                    </svg>
            ...
            {clientSecret ? (<div className="mt-8">
                <react_stripe_js_1.Elements stripe={stripePromise} options={{
                clientSecret,
                appearance: {
                    theme: 'stripe',
                    variables: {
              ...
              </div>) : (<button onClick={handleSubscribe} disabled={isProcessing || hasSubscription} className={`mt-8 block w-full px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-bold text-center transition-all duration-300 ${hasSubscription
                ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                : "bg-white text-green-700 hover:bg-green-50"} ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>
                {isProcessing ? '処理中...' : hasSubscription ? "すでに登録済み" : "プランに登録する"}
              </button>)}
          </framer_motion_1.motion.div>

          {/* エリートプラン */}
          <framer_motion_1.motion.div className="relative p-8 bg-gradient-to-b from-amber-50 to-amber-100 border border-amber-200 rounded-2xl shadow-md flex flex-col transform">
            <div className="flex-1 relative">
              <div className="flex items-center">
                ...
                <h3 className="text-3xl font-semibold text-amber-800">{subscriptions_1.SUBSCRIPTION_PLANS.ELITE.name}</h3>
              </div>
              <p className="mt-4 flex items-baseline text-amber-900">
                <span className="text-5xl font-extrabold tracking-tight">¥{subscriptions_1.SUBSCRIPTION_PLANS.ELITE.price.toLocaleString()}</span>
                <span className="ml-1 text-xl font-semibold">/月</span>
              </p>
              <p className="mt-6 text-amber-800">
                {subscriptions_1.SUBSCRIPTION_PLANS.ELITE.description}
              </p>

              <ul role="list" className="mt-6 space-y-4">
                {subscriptions_1.SUBSCRIPTION_PLANS.ELITE.features.map((feature, index) => (<li key={index} className="flex">
                    <svg className="flex-shrink-0 w-6 h-6 text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/>
                    </svg>
            ...
            <button disabled={true} className="mt-8 block w-full px-6 py-3 border border-amber-300 rounded-md shadow-sm text-base font-medium text-center bg-white text-amber-700 opacity-60 cursor-not-allowed">
              ご応募多数につき受付停止中
            </button>
