...
exports.default = ExamDetailPage;
const react_1 = require("react");
const navigation_1 = require("next/navigation");
const AuthContext_1 = require("@/app/contexts/AuthContext");
const useToast_1 = require("@/app/hooks/useToast");
const link_1 = __importDefault(require("next/link")); // Linkを追加
const LoadingSpinner_1 = __importDefault(require("@/app/components/LoadingSpinner")); // ローディングスピナーを追加
const exams_1 = require("@/app/lib/exams"); // Exam を削除
const framer_motion_1 = require("framer-motion"); // motion をインポート
...
function ExamDetailPage({ params }) {
    const router = (0, navigation_1.useRouter)();
    const { user, loading } = (0, AuthContext_1.useAuth)(); // loading は認証状態のローディング
    const { showToast } = (0, useToast_1.useToast)();
    const [exam, setExam] = (0, react_1.useState)(null); // Exam を ExamData に変更
    // purchaseStatus は不要なので削除
    // const [purchaseStatus, setPurchaseStatus] = useState<'none' | 'pending' | 'completed' | 'failed'>('none');
    const [isLoading, setIsLoading] = (0, react_1.useState)(true); // ページ全体のローディング状態
    (0, react_1.useEffect)(() => {
        const fetchExam = async () => {
            setIsLoading(true);
            try {
                // getExam 関数を直接呼び出す
                const data = await (0, exams_1.getExam)(params.id);
    ...
    return (<div className="bg-gray-50 min-h-screen pb-12"> {/* 背景色とpaddingを追加 */}
      {/* 新しいヘッダーセクション */}
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-12">
        <div className="container mx-auto px-4">
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.5 }} className="mt-4 flex items-center space-x-4 text-blue-200">
            ...
            {exam.isFree && (<span className="inline-flex items-center bg-gradient-to-r from-yellow-400 to-yellow-600 text-white text-xs font-medium px-2.5 py-0.5 rounded-full">
                 無料
               </span>)}
      ...
      <div className="container mx-auto px-4 py-8 -mt-10"> {/* ネガティブマージンでヘッダーと重ねる */}
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="bg-white rounded-lg shadow-xl p-6 md:p-8">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg mb-8">
            ...
            <ul className="ml-8 mt-3 list-disc text-sm text-blue-700 space-y-2">
              ...
              {exam.type === 'TOEFL' || exam.type === 'IELTS' || exam.type === 'EIKEN' ? (<li>スピーキング・ライティングセクションでは、マイク・キーボードが必要になります。事前にデバイスの接続と動作をご確認ください。</li>) : null}
              {exam.type === 'TOEIC' && (<li>リスニングセクションとリーディングセクションに分かれています。セクション間の移動は指示に従ってください。</li>)}
